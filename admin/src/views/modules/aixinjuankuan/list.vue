<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px 0","fontSize":"14px","color":"#666","background":"#f6f6f6"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 30px","margin":"0 0 0 30px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 30px)","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('aixinjuankuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('aixinjuankuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"calc(100% - 30px)","padding":"0 30px 30px","margin":"0px 0 0 30px","borderRadius":"0px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('aixinjuankuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('aixinjuankuan','我要捐款')" @click="juankuanxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								我要捐款
							</el-button>
							<el-button class="edit" v-if=" isAuth('aixinjuankuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								更新
							</el-button>

							<el-button class="view" v-if="isAuth('aixinjuankuan','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('aixinjuankuan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<juankuanxinxi-cross-add-or-update v-if="juankuanxinxiCrossAddOrUpdateFlag" :parent="this" ref="juankuanxinxiCrossaddOrUpdate"></juankuanxinxi-cross-add-or-update>
...
import juankuanxinxiCrossAddOrUpdate from "../juankuanxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				juankuanxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			juankuanxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.juankuanxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','aixinjuankuan');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.juankuanxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.juankuanxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	this.$router.push({path:'/discussaixinjuankuan',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	aixinjuankuanstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'aixinjuankuan/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
