<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px 0","fontSize":"14px","color":"#666","background":"#f6f6f6"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"20px 30px","margin":"0 0 0 30px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 30px)","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 0 10px","textAlign":"left","flexWrap":"wrap","display":"flex","width":"auto","fontSize":"inherit","order":"1"}' >
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">捐款项目</label>
						<el-input v-model="searchForm.juankuanxiangmu" placeholder="捐款项目" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">是否支付</label>
				...
				<el-row class="actions" :style='{"margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('juankuanxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('juankuanxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('juankuanxinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"border":"0px solid #eee","width":"calc(100% - 30px)","padding":"0 30px 30px","margin":"0px 0 0 30px","borderRadius":"0px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="juankuanxiangmu"
						label="捐款项目">
						<template slot-scope="scope">
							{{scope.row.juankuanxiangmu}}
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="juanzengduixiang"
						label="捐赠对象">
						<template slot-scope="scope">
							{{scope.row.juanzengduixiang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="juankuanjine"
						label="捐款金额">
						<template slot-scope="scope">
							{{scope.row.juankuanjine}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="juankuanshijian"
						label="捐款时间">
						<template slot-scope="scope">
							{{scope.row.juankuanshijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="yonghuzhanghao"
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('juankuanxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('juankuanxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('juankuanxinxi','公示')" @click="juankuangongshiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								公示
							</el-button>
							<el-button class="edit" v-if=" isAuth('juankuanxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('juankuanxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<juankuangongshi-cross-add-or-update v-if="juankuangongshiCrossAddOrUpdateFlag" :parent="this" ref="juankuangongshiCrossaddOrUpdate"></juankuangongshi-cross-add-or-update>
...
import juankuangongshiCrossAddOrUpdate from "../juankuangongshi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				juankuangongshiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			juankuangongshiCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			juankuangongshiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.juankuangongshiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','juankuanxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.juankuangongshiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.juankuangongshiCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','juankuanxinxi');
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.juankuanxiangmu!='' && this.searchForm.juankuanxiangmu!=undefined){
            params['juankuanxiangmu'] = '%' + this.searchForm.juankuanxiangmu + '%'
          }
	...
	juankuanxinxistatusChange(e,row){
		if(row.status==0){
