...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}'  label="商家账号" prop="shangjiazhanghao">
            <el-select v-model="ruleForm.shangjiazhanghao" placeholder="请选择商家账号" :disabled=" false  ||ro.shangjiazhanghao" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="图片" v-if="type!='cross' || (type=='cross' && !ro.tupian)" prop="tupian">
            <file-upload
            tip="点击上传图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' class="upload" v-else label="图片" prop="tupian">
                <img v-if="ruleForm.tupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.tupian.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="申请原因" prop="shenqingyuanyin">
            <el-input v-model="ruleForm.shenqingyuanyin" 
                placeholder="申请原因" clearable :disabled=" false  ||ro.shenqingyuanyin"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="申请时间" prop="shenqingshijian">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="反馈结果" v-if="type!='cross' || (type=='cross' && !ro.fankuijieguo)" prop="fankuijieguo">
            <file-upload
            tip="点击上传反馈结果"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.fankuijieguo?ruleForm.fankuijieguo:''"
            @change="fankuijieguoUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' class="upload" v-else label="反馈结果" prop="fankuijieguo">
                <img v-if="ruleForm.fankuijieguo.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.fankuijieguo.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fankuijieguo.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="处理时间" prop="chulishijian">
              <el-date-picker
				  :disabled=" false  ||ro.chulishijian"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="处理内容" prop="chulineirong">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='fankuijieguo'){
              this.ruleForm.fankuijieguo = obj[o].split(",")[0];
              this.ro.fankuijieguo = true;
              continue;
            }
            if(o=='chulineirong'){
        ...
        this.$http.get('option/shangjia/shangjiazhanghao', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.shangjiazhanghaoOptions = res.data.data;
          }
        });
        this.leixingOptions = "退货,换货,其他服务".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shouhoufankui/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shouhoufankui/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shouhoufankui/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      fankuijieguoUploadChange(fileUrls) {
          this.ruleForm.fankuijieguo = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
