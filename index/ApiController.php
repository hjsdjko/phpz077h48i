...
class ApiController extends CommonController
{
    ...
    public function download()
    {
        ...
        header('location:http://' . $_SERVER['HTTP_HOST'] . '/phpz077h48i/upload/' . $fileName);
        exit; 
    }
    /**
     * 获取某表的某个字段列表接口
    ...
    public function group($tablesName,$columnName){
        ...
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tokens['tablename'] == 'shangpinkucun'){
            ...
            if($tokens['tablename'] == 'juankuanxinxi'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'juankuangongshi'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'shouhoushenqing'){
                if($tokens['tablename'] == 'shangjia'){
                    $where .= " and `shangjiazhanghao` = '".$tokens['username']."' ";
                }
            }
            ...
            if($tokens['tablename'] == 'shouhoufankui'){
                if($tokens['tablename'] == 'shangjia'){
                    $where .= " and `shangjiazhanghao` = '".$tokens['username']."' ";
                }
            }
    ...
    public function value($tableName,$xColumnName,$yColumnName,$tjType){
        ...
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            ...
            if($tableName == 'juankuanxinxi'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'juankuangongshi'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'shouhoushenqing'){
                if($tokens['tablename'] == 'shangjia'){
                    $where .= " and `shangjiazhanghao` = '".$tokens['username']."' ";
                }
            }
