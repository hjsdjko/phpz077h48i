...
class ShangjiaController extends CommonController {
    ...
    public function login(){
        ...
        $sql = "select * from `shangjia` where `shangjiazhanghao` = '$username'";
        $result = table_sql($sql);
          if ($result->num_rows > 0) {//账号数据
            while($row = $result->fetch_assoc()) {
                if($row['status']==1){
                    $data = ['code' => 500, 'msg' => "账号已锁定，请联系管理员审核!"];
                    exit(json_encode($data));
                }
                if($row['mima']!=$password){//密码错误
                    $passwordwrongnum = $row['passwordwrongnum']+1;
                    if($passwordwrongnum>= 3){
                        $sql = "update shangjia set passwordwrongnum = '$passwordwrongnum',status = '1' where shangjiazhanghao = '".$username."'";
                    }else{
                        $sql = "update shangjia set passwordwrongnum = '$passwordwrongnum' where shangjiazhanghao = '".$username."'";
                    }
                    $result = table_sql($sql);
                    $data = ['code' => 10028, 'msg' => "账号或密码错误!"];
                    exit(json_encode($data));
                }
            }
        }else{
    ...
    public function shBatch(){
        ...
        preg_match_all('/\d+/',$ids,$arr);
        $str = implode(',',$arr[0]);//拼接字符，
        $sql = "update shangjia set sfsh = '".$_GET['sfsh']."', shhf='".$_GET['shhf']."' WHERE id in(".$str.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
